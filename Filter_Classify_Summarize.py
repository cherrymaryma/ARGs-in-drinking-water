...
while True:
    try:
        ...
        if not os.path.exists(filename):
            filename=raw_input("Tips: invalid [blast-output],pls re-enter [blast.txt]: ")
        if not os.path.exists(database):
            database=raw_input("Tips: invalid [db_directory], pls re-enter [db_directory]: ")
        break
    
    except:
        Parameters=raw_input("Enter parameters: names of [blast-output],[db_directory],[similarity_range](%),[align_num] sepeated by Space: ")
...
output3=open('Results_'+filename1+'_'+similarity_range+'\\'+'Merged_Classified_'+filename1+'.txt','w')
...
ID_collection=[]
Folder_collection=[]
for line in open('Results_'+filename1+'_'+similarity_range+'\\'+'Merged_Classified_'+filename1+'.txt','r'):
